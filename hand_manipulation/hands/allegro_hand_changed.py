...
class AllegroHand:
    ...
    def apply_force(self, digit: str, force: float):
        """Apply a force to the specified digit."""
        assert digit in self.digit_tips.keys(), f"Invalid digit: {digit}, must be one of {self.digit_tips.keys()}"

        # Get the joint speeds that correspond to the force
        joint_speeds = np.zeros_like(self.data.qvel)
        joint_speeds[self.actuators.joints(digit)] = force

        # Apply the speeds to the actuators
        self.data.ctrl[self.actuators.actuators(digit)] = self.actuators.get_actuator_speeds(joint_speeds)